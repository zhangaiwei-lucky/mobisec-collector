...
\textbf{无需任何权限即可 200Hz 采样}。本 App 利用传感器噪声生成跨 Factory Reset
的设备硬件指纹，并实现无权限活动识别——这是 Android 权限沙箱的系统性盲区。
\end{mdframed}
...
生成指纹哈希如 \texttt{3A7F1B2C9D4E5F60}，\textbf{跨App一致、重置后不变}。
\end{mdframed}
...
\noindent\textbf{安全价值}：Cookie/广告ID可被清除，但传感器指纹
\textbf{无法被用户消除}，即使更换SIM卡、重置设备、使用VPN，
广告商和追踪者仍可跨App关联同一物理设备。
...
\noindent 读取 \texttt{/sys/fs/selinux/enforce} 和反射调用 \texttt{android.os.SELinux}
获取当前MAC策略状态。Permissive模式下，SEAndroid的强制访问控制完全失效，
...
检测设置了 \texttt{usesCleartextTraffic=true} 的应用（MITM风险）。
...
为 Intent 注入和 Content Provider 路径遍历提供攻击面。
防御方向：传感器访问纳入权限管控 + 传感器数据噪声注入（Noise Injection） +
强制组件导出最小化原则。
